...
#
# Ported verbatim from JaneliaSciComp/marimo_ai_sandbox's
# container/caddy-lib.sh -- nothing in here is marimo- or Mojo-specific.
...
# caddy_generate_cert (never Caddy's own internal-CA issuer -- that issuer
# shells out to `sudo` on first use, which hangs on a host with no
...
#   SERVICE_PID -- the backend service's PID (ttyd here); the publisher
#                  gives up if either this or Caddy dies first.
#
